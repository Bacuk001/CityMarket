...
export class PriceService implements IPriceService {
    ...
    getPriceByCategoryAndMarketWithProduct(directionSorting: string, pageNumber: number, sizePage: number) {
        let marketId = this.marketService.getSelectMarket().id;
        let categoryId = this.categoryService.getSelectedCategory().id;
        const urlPrices = URL + 's/market/' + marketId + '/category/' + categoryId + '/direction/' + directionSorting
            + '/pageSize/' + sizePage + '/page/' + pageNumber;
        return new Promise((response, reject) => {
            this.http.get(urlPrices)
                .subscribe(resp => response(this.formingListProducts(resp.json())),
                    error => reject(NOT_DOWNLOAD));
        });
    }

    /**
     * Method returns the price of products in the store in the category.
     * @returns {Promise<T>}
     */
    getCountProductCategoryAndMarket() {
        let marketId = this.marketService.getSelectMarket().id;
        let categoryId = this.categoryService.getSelectedCategory().id;
        const urlPrices = URL + 's/market/' + marketId + '/category/' + categoryId + '/count';
        return new Promise((response, reject) => {
            this.http.get(urlPrices)
                .subscribe(resp => response(resp.json()),
                    error => reject(NOT_DOWNLOAD));
    ...
    private formingListProducts(prices: Price[]) {
        let products = new Array(prices.length);
        for (let index = 0; index < prices.length; index++) {
            prices[index].product.priceView = prices[index].price.toString();
            products[index] = prices[index].product;
        }
        return products;
    }
